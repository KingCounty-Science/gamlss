...
outputQ.waterYear2 <- outputQ %>% filter(waterYear2<2024) %>% group_by(site_no,basin,waterYear2,Season2) %>% summarise(pkQ = max(Q*35.3147,na.rm=T)) %>% 
  ungroup() %>% 
...
pks <- tmp %>% group_by(waterYear2) %>% summarize(Q = max(OND_Q,JFM_Q,AMJ_Q,JAS_Q)) %>% ungroup()
...
for (i in 1:iter){
  ...
  suppressMessages(dum <- pred.pks %>% group_by(iter,waterYear2) %>% summarize(Q = max(Q)) %>% ungroup())
